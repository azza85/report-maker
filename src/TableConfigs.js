...
export const TableConfigs = (props) => {
  ...
  console.log(' form.listData => ', {form, listFields})
  // const keySelect = form.listData !== undefined ? deepKeys(listFields[form.listData]).map(item => ({value: `memberDetails.${item}`, label: toTitleCase(item.replace(/[._]/g, ' ')).replace(/Id/g, 'ID')})) : []
  /*
  const keySelect = form.listData !== undefined
    ? deepKeys(listFields[form.listData])
      .map(item => ({value: `memberDetails.${item}`, label: toTitleCase(item.replace(/[._]/g, ' ')).replace(/Id/g, 'ID')})) : []
  */
  const keySelect = form.listData !== undefined
    ? Object.keys(listFields[form.listData]).map(item => ({value: `memberDetails.${item}`, label: toTitleCase(item.replace(/[._]/g, ' ')).replace(/Id/g, 'ID')})) : []
  ...
  return <Fragment>
    ...
    <div style={{display: 'flex', justifyContent: 'space-between'}}>
      ...
      <div>
        <h4>Column Name</h4>
        {rangeInclusive(form.cols).map(item => <PopupText key={item} question={`Col ${item}`} name={`col${item}`} value={form[`col${item}`]} handleChange={handleChange} />)}
        <h4>Column Value</h4>
